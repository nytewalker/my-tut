...
function Counter() {
  ...
  return (
    <div className="flex gap-5 text-xl mt-5  item-center justify-center select-none">
      <button
        onClick={() => dispatch({ value: "decrement", payload: 1 })}
        className="bg-slate-400 px-2 text-slate-50 flex items-center justify-center rounded-lg"
      >
        -
      </button>
      <p className="flex items-center justify-center">Count : {state.count}</p>
      <button
        onClick={() => dispatch({ value: "increment", payload: 1 })}
        className="bg-slate-400 px-2 text-slate-50 flex items-center justify-center rounded-lg"
      >
        +
      </button>
      <button
        onClick={() => dispatch({ value: "reset" })}
        className="bg-slate-400 px-2 text-slate-50 flex items-center justify-center rounded-lg"
      >
        Reset
      </button>
